...
class UltimateTermConverter:
    def __init__(self, root):
        self.root = root
        self.root.title("🌈 全能术语转换中心 Ultra")
        ...
        self.author_id = tk.StringVar(value="USER")
    ...
    def setup_ui(self):
        ...
        ttk.Label(main_container, text="术语库工作台", style="Header.TLabel").pack(anchor="w", pady=(0, 15))
        ...
        ttk.Label(panel_format, text="✨ 选择导出格式 (兼容多种文件格式智能解析):", font=("Microsoft YaHei", 10, "bold")).grid(row=0, column=0, columnspan=4, sticky="w", pady=(0, 10))
    ...
    def parse_tbx_xml(self, filepath):
        try:
            ...
            self.log(f"⚠️ 文件格式非标准 ({str(e).split(':')[0]})，尝试使用兼容模式读取...")
            self.log("🔄 自动切换至【智能内容识别模式】...")
    ...
    def parse_xml_fallback(self, filepath):
        ...
        raise ValueError("未能从文件中提取到有效的术语对照内容，请检查文件格式是否正确。")
    ...
    def start_conversion(self):
        ...
        try:
            ...
            self.log(f"✅ 成功读取并解析 {len(data)} 条术语数据")
            ...
            messagebox.showinfo("转换成功", f"转换已完成！\n\n• 成功读取：{len(data)} 条术语\n• 导出格式：{cnt} 种\n\n文件已保存至指定文件夹。")
            
        except Exception as e:
            # 用户友好的错误提示
            self.log(f"❌ 任务中断: {str(e)}")
            messagebox.showerror("转换未能完成", f"处理过程中出现以下问题：\n\n{str(e)}\n\n建议：\n• 请确认文件格式为 CSV、Excel、TBX 或 XML\n• 检查文件内容是否包含术语对照数据\n• 如问题持续，请尝试另存为新文件后再导入")
